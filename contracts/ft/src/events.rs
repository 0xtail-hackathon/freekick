...
/// Data to log for an FT deposit event. To log this event, call [`.emit()`](FtDeposit::emit).
#[must_use]
#[derive(Serialize, Debug, Clone)]
...
impl FtDeposit<'_> {
    ...
    /// Emits an FT deposit event, through [`env::log_str`](near_sdk::env::log_str),
    /// where each [`FtDeposit`] represents the data of each deposit.
...
/// Data to log for an FT withdraw event. To log this event, call [`.emit()`](FtWithdraw::emit).
#[must_use]
#[derive(Serialize, Debug, Clone)]
...
impl FtWithdraw<'_> {
    ...
    /// Emits an FT withdraw event, through [`env::log_str`](near_sdk::env::log_str),
    /// where each [`FtWithdraw`] represents the data of each withdraw.
